...
const Details = () => {
  ...
  useEffect(() => {
    async function checkIfDetailsExist() {
      try {
        ...
        if (res.data?.name && res.data?.phoneNo && res.data?.roomNo) {
  ...
  async function addDetails() {
    ...
    const roomVal = roomNoRef.current?.value?.trim()

    if (!nameVal || !phoneVal || !roomVal) {
    ...
    try {
      ...
      const response = await axios.post(
        "https://hostel-code.onrender.com/details",
        {
          name: nameVal,
          phoneNo: phoneVal,
          roomNo: roomVal,
      ...
      <div className='border border-slate-800 h-100 w-80 rounded-md'>
        ...
        <div className='flex justify-center items-center flex-col'>
          <input
            ...
            className='ml-5 m-4 pr-20 border-b placeholder:text-shadow-slate-300 focus:outline-0 focus:border-b-2'
          />
          <input
            type="text"
